...
<template>
  <div flex="~ 1" gap-5 overflow-hidden>
    <div flex="~ col" gap-3 justify-around rounded-lg bg-black bg-op40>
      <RouterLink
        v-for="(i, index) in list" :key="i"
        flex gap-3 p5 rounded
        :to="{ query: { sidebar: index || undefined } }"
        :style="($route.query.sidebar || '0') === `${index}` && 'background: linear-gradient(90deg, #1A95FF 0%, rgba(26, 149, 255, 0.00) 100%);'"
      >
        <span>0{{ index + 1 }}</span>
        <span>{{ i }}</span>
      </RouterLink>
